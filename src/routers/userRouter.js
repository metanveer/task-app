...
const { sendWelcomeMail, sendDeleteMail } = require("../email/account");
...
router.post("/users", async (req, res) => {
  ...
  try {
    await user.save();
    sendWelcomeMail(user.name, user.email); //Working!!
...
const upload = multer({
  ...
  fileFilter(req, file, cb) {
    if (!file.originalname.match(/\.(jpg|jpeg|png)$/)) {
      return cb(
        new Error(
          "Pls upload an image (jpg or jpeg or png format); not over 1 MB"
        )
      );
    }
    cb(undefined, true);
...
router.delete("/users/me", auth, async (req, res) => {
  try {
    ...
    sendDeleteMail(req.user.name, req.user.email); //working!!
    res.send(req.user);
  } catch (e) {
    res.send(e);
  }
